package it.auties.compiler.tree.statement;

import it.auties.compiler.tree.expression.ExpressionTree;
import it.auties.compiler.tree.model.Tree;
import it.auties.compiler.tree.model.TreeTag;
import it.auties.compiler.tree.source.BodyTree;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NonNull;
import lombok.experimental.Accessors;

@AllArgsConstructor
@Data
@Accessors(fluent = true)
public final class ForEachTree implements StatementTree {
    private int position;

    @NonNull
    private Tree owner;

    @NonNull
    private VariableTree initializedVariable;

    @NonNull
    private ExpressionTree iterableTarget;

    @NonNull
    private BodyTree body;

    @Override
    public TreeTag tag() {
        return TreeTag.FOR_EACH;
    }
}
